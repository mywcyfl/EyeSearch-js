/**
 *
 * The Eye-Search (js version)
 * Author: wangxinwen, Sun Yat-sen University
 * contact me at email: mywcyfl@163.com
 *
 */

//-----------------------------------------------------------------
//--Area of Constant, which is the parameter of algorithm
//-----------------------------------------------------------------
var config = {
	// 最大一级分辨率下网格每一边上被划分为的子网格数
	// 网格被划分为的子网格总数M等于该值的平方
	CHILD_GRID_CNT_ONE_SIDE_MAX : 8,
	// 迭代次数（每迭代一次，分辨率长和宽上各降低一半）
	ITERATE_CNT : 2,
	// 分层筛选比
	SCREEN_RATIO : 0.5,
	// 分割因子
	 ...
	 *	_ _
	 */
	FEATURE_LU_RU : parseInt('0011', 2),
	/**
	 * topology appearance
	 *	_ _
	 *	* *
	 */
	FEATURE_LD_RD : parseInt('1100', 2),
	/**
	 * topology appearance
	 *	* _
	 *	* _
	 */
	FEATURE_LU_lD : parseInt('0101', 2),
	/**
	 * topology appearance
	 *	_ *
	 *	_ *
	 */
	FEATURE_RU_RD : parseInt('1010', 2),
	/**
	 * topology appearance
	 *	* _
	 *	_ *
	 */
	FEATURE_LU_RD : parseInt('1001', 2),
	/**
	 * topology appearance
	 *	_ *
	 *	* _
	 */
	FEATURE_LD_RU : parseInt('0110', 2),
	/**
	 * topology appearance
	 *	* _
	 *	_ _
	 */
	FEATURE_LU : parseInt('0001', 2),
	/**
	 * topology appearance
	 *	_ _
	 *	* _
	 */
	FEATURE_LD : parseInt('0100', 2),
	/**
	 * topology appearance
	 *	_ *
	 *	_ _
	 */
	FEATURE_RU : parseInt('0010', 2),
	/**
	 * topology appearance
	 *	_ _
	 *	_ *
	 */
	FEATURE_RD : parseInt('1000', 2)
};
 ...
 */
EyeSearch = function(){
	this.database = TemplateService;
	this.database.readDatabase();
}

EyeSearch.prototype.retrieval = function(sketchPoints){
	var sketch = new Sketch('unkonw', sketchPoints);
	return this.database.getMostSimilarByWeight(sketch);
	// return this.database.getMostSimilarPymid(sketch, config.SCREEN_RATIO);
}
 ...
 * sketch	待规范草图（点集合）
 */
Normalizer.normalize = function(sketch){
 ...
 * resampleCnt	重采样点个数
 */
Normalizer.__resampleNormalize = function(sketch, resampleCnt){
	// 重采样后的草图，第一个点也从之前的第一个点开始
	var newSketch = [sketch[0]];
	// 重采样后两点间应该具有的间距
	var interval = Normalizer.__pathLength(sketch) / (resampleCnt - 1);
	// 线段分割长度，当两点间距超过此值时，认为这两点属于两条线段
	var boundingBox = Normalizer.__boundingBox(sketch);
	var segmentJudgeLen = Math.max(boundingBox.WID, boundingBox.HGT) / config.SEGMENT_JUDGE_FACTOR;

	var dist = 0;
	for(var i = 1; i < sketch.length; i++){
		var d = pointDist(sketch[i-1], sketch[i]);
		if(d >= segmentJudgeLen){
			dist = 0
			continue;
		} else if((dist + d) >= interval){
			var qx = sketch[i-1].X + ((interval-dist)/d)*(sketch[i].X-sketch[i-1].X);
			var qy = sketch[i-1].Y + ((interval-dist)/d)*(sketch[i].Y-sketch[i-1].Y);

			newSketch.push(new Point(qx, qy));
			// 低效的函数，是的
			sketch.splice(i, 0, new Point(qx, qy));
			dist = 0;
		} else {
			dist += d;
		}
 ...
 * sketch		待规范化草图（点集合）
 */
Normalizer.__rotateNormalize = function(sketch){
	var cp = Normalizer.__centroid(sketch);
	var furthestDist	= 0;
	var furthestPoint	= cp;

	// got the furthest point
	var dist = 0;
	for(var i = 0; i < sketch.length; i++){
		dist = pointDist(cp, sketch[i]);
		if(dist > furthestDist){
			furthestPoint = sketch[i];
		}
	}

	// rotate
	var theta = Math.atan2(cp.Y-furthestPoint.Y, cp.X-furthestPoint.X);
	var newSketch = Normalizer.__rotateBy(sketch, theta);
 ...
 *
 */
Normalizer.__scaleNormalize = function(sketch, standardSideLen){
	var newSketch = [];
	var boundingBox = Normalizer.__boundingBox(sketch);
	var scaleW = standardSideLen / boundingBox.WID;
	var scaleH = standardSideLen / boundingBox.HGT;

	for (var i = 0; i < sketch.length; i++){
		var qx = sketch[i].X * scaleW;
		var qy = sketch[i].Y * scaleH;
		newSketch.push(new Point(qx, qy));
	}
 ...
 */
Normalizer.__translateNormalize = function(sketch){
	var boundingBox = Normalizer.__boundingBox(sketch);
	for(var i = 0; i < sketch.length; i++){
		sketch[i].X -= boundingBox.X;
		sketch[i].Y -= boundingBox.Y;
	}
 ...
 * 求一个草图其轮廓曲线的总长度
 */
Normalizer.__pathLength = function(sketch){
	var pathLen = 0.0;
	var boundingBox = Normalizer.__boundingBox(sketch);
	var segmentJudgeLen = Math.max(boundingBox.WID, boundingBox.HGT) / config.SEGMENT_JUDGE_FACTOR

	for(var i = 1; i < sketch.length; i++){
		var dist = pointDist(sketch[i-1], sketch[i]);
		if(dist <= segmentJudgeLen){
			pathLen += dist;
		}
	}
 ...
 * 求一个轮廓的集合中心
 */
Normalizer.__centroid = function(sketch){
	var x = 0.0, y = 0.0;
	for (var i = 0; i < sketch.length; i++){
		x += sketch[i].X;
		y += sketch[i].Y;
 ...
 * 将轮廓按照一定角度整体旋转
 */
Normalizer.__rotateBy = function(sketch, radians){
	var c = Normalizer.__centroid(sketch);
	var cos = Math.cos(radians);
	var sin = Math.sin(radians);
	
	var newSketch = [];
	for (var i = 0; i < sketch.length; i++){
		var qx = (sketch[i].X-c.X) * cos - (sketch[i].Y-c.Y)*sin + c.X
		var qy = (sketch[i].X-c.X) * sin + (sketch[i].Y-c.Y)*cos + c.Y;
		newSketch.push(new Point(qx, qy));
	}
 ...
 * 求一个草图其轮廓的最小外包围框
 */
Normalizer.__boundingBox = function(sketch){
 ...
 * 执行特征提取
 */
FeatureExtractor.extract = function(sketch){
	var feature = [];
	// cg is short for child grid
	var curCGCntOneSide = config.CHILD_GRID_CNT_ONE_SIDE_MAX;
	// cg is short for child grid
	var curCGSideLen	= config.STANDARD_GRID_SIDE_LEN / curCGCntOneSide;
	// extract the feature in max resolution
	feature[config.ITERATE_CNT-1] = FeatureExtractor.__extractByPath(sketch, curCGSideLen, curCGCntOneSide);

	// extract feature of other resolution iteratively
	var i = config.ITERATE_CNT - 2;
	while(i >= 0){
		curCGCntOneSide /= 2;
		curCGSideLen = config.STANDARD_GRID_SIDE_LEN / curCGSideLen;
		feature[i] = FeatureExtractor.__extractByPath(sketch, curCGSideLen, curCGCntOneSide);
		i--;
	}
 ...
 * 沿着曲线不断提取特征值
 * sketch		待提取草图
 * cgSideLen	子单元格边长(正方形）
 * cgCntOneSide 网格每边有多少个子单元格
 */
FeatureExtractor.__extractByPath = function(sketch, cgSideLen, cgCntOneSide){
	var layerFeature = [];
	var thisGrid = [-1, -1];
	var lastGrid = [-1, -1];
	var curCGTag = [0, 0, 0, 0];
	var xInCG = 0;
	var yInCG = 0;

	var i = 0;
	for(; i < sketch.length; i++){
		thisGrid[0] = parseInt(sketch[i].X/cgSideLen);
		if(thisGrid[0] >= cgCntOneSide){
			thisGrid[0] -= 1;
		}
		thisGrid[1] = parseInt(sketch[i].Y/cgSideLen);
		if(thisGrid[1] >= cgCntOneSide){
			thisGrid[1] -= 1;
		}

		if(thisGrid[0] !== lastGrid[0] || thisGrid[0] !== lastGrid[1]){
			if(-1 !== lastGrid[0] && -1 !== lastGrid[1]){
				// now enter a new child grid
				xInCG = sketch[i-1].X - lastGrid[0]*cgSideLen;
				yInCG = sketch[i-1].Y - lastGrid[1]*cgSideLen;

				// now calculate the cg feature of sketch in last cg
				FeatureExtractor.__recordTopoInfo(xInCG, yInCG, cgSideLen, curCGTag);
				layerFeature[lastGrid[1]*cgCntOneSide + lastGrid[0]] |= FeatureExtractor.__getCGFeature(curCGTag);
			}

			curCGTag = [0, 0, 0, 0];
			lastGrid = [thisGrid[0], thisGrid[1]];

			// record the topo info of this new grid which just be entered in
			xInCG = sketch[i].X - thisGrid[0]*cgSideLen;
			yInCG = sketch[i].Y - thisGrid[1]*cgSideLen;
			FeatureExtractor.__recordTopoInfo(xInCG, yInCG, cgSideLen, curCGTag);
		} else {
 ...
 */
FeatureExtractor.__recordTopoInfo = function(xInGrid, yInGrid, gridSideLen, cgTag){
	var areaLen = gridSideLen / 2;

	if(xInGrid < areaLen && yInGrid < areaLen){
		cgTag[0] = true;
	} else if (xInGrid >= areaLen && yInGrid < areaLen){
		cgTag[1] = true;
	} else if (xInGrid < areaLen && yInGrid >= areaLen){
		cgTag[2] = true;
	} else {
...
/**
 * 根据记录好的区域特性得到草图在此子单元格中的拓扑特征
 * cgTag 记录好的草图在某子单元格的区域分布特性
 */
FeatureExtractor.__getCGFeature = function(cgTag){
	var cgFeature = 0;
	
	if(true === cgTag[0] && true === cgTag[1]){
		/**
		 * topo appearance
		 *	* *
		 *	_ _
	...
	} else if(true === cgTag[2] && true === cgTag[3]){
		/**
		 * topo appearance
		 * _ _
		 * * *
	...
	} else if(true === cgTag[0] && true === cgTag[2]){
		/**
		 * topo appearance
		 * * _
		 * * _
	...
	} else if(true === cgTag[1] && true === cgTag[3]){
		/**
		 * topo appearance
		 * _ *
		 * _ *
	...
	} else if(true === cgTag[0] && true === cgTag[3]){
		/**
		 * topo appearance
		 * * _
		 * _ *
	...
	} else if(true === cgTag[1] && true == cgTag[2]){
		/**
		 * topo appearance
		 * _ *
		 * * _
 ...
 * 计算某层分辨率下两个特征向量的相似度
 */
SimilarityCalculator.calculateSimiInOneResolution = function(layerFeature1, layerFeature2){
	var score = 0;

	for(var i = 0; i < layerFeature1.length; i++){
		score += SimilarityCalculator.__getCGFeatureSimilarity(layerFeature1[i], layerFeature2[i]);
	}
 ...
 * 计算两个子单元格特征向量的相似度
 */
SimilarityCalculator.__getCGFeatureSimilarity = function(cgFeature1, cgFeature2){
	var score = 0;

	var bit1Similarity = ((parseInt('0001', 2)&cgFeature1)^(parseInt('0001', 2)&cgFeature2)) > 0?-1:0;
	var bit2Similarity = ((parseInt('0010', 2)&cgFeature1)^(parseInt('0010', 2)&cgFeature2)) > 0?-1:0;
	var bit3Similarity = ((parseInt('0100', 2)&cgFeature1)^(parseInt('0100', 2)&cgFeature2)) > 0?-1:0;
	var bit4Similarity = ((parseInt('1000', 2)&cgFeature1)^(parseInt('1000', 2)&cgFeature2)) > 0?-1:0;
 ...
 * sketch 待检索草图
 */
TemplateService.getMostSimilarByWeight = function(sketch){
	var result = [];
	for(var i = 0; i < this.templateDB.length; i++){
		var childGridCnt = config.CHILD_GRID_CNT_ONE_SIDE_MAX*config.CHILD_GRID_CNT_ONE_SIDE_MAX;

		var score = 0;
		var s = 0;
		for(var j = config.ITERATE_CNT -1; j>=0; j--){
			s = SimilarityCalculator.calculateSimiInOneResolution(sketch.feature[j], this.templateDB[i].feature[j], childGridCnt);
			s *= (config.CHILD_GRID_CNT_ONE_SIDE_MAX*config.CHILD_GRID_CNT_ONE_SIDE_MAX)/childGridCnt;
			score += s
			// 降低一级分辨率，长和宽方向上子格数各降低一半，总数为原来的1/4
			childGridCnt /= 4;
		}
		result[i] = new Result(this.templateDB[i].name, score);
	}

	result.sort(function(a, b){return a.score < b.score?1:-1;});

	console.log(result);

	return new Result('star', 1.0);
};
 ...
 * ratio	分层筛选比
 */
TemplateService.getMostSimilarPymid = function(sketch, ratio){
	return new Result('triangle', 1.0);
}
 ...
 * Sketch class
 */
function Sketch(name, points){
    this.name = name;
    this.sketch = Normalizer.normalize(points);
	this.feature = FeatureExtractor.extract(this.sketch);
}
...
//-----------------------------------------------------------------
/**
 * Get the Euclidean distance of two point
 */
